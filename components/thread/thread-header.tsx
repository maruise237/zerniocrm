...
export function ThreadHeader({
  ...
  // Étiquettes posées par l'agent IA (support-auto…) : affichées à côté du
  // numéro. La conversation n'expose pas de tags — on les prend des contacts.
  const { tagsFor } = useContactTags();
  const contactTags = tagsFor(conversation.participantId);
  ...
  return (
    <header className="flex h-14 flex-none items-center gap-2.5 border-b border-[var(--chat-border)] bg-[var(--chat-surface)] px-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        {(contactSub || lastActive || contactTags.length > 0) && (
          <p className="mt-0.5 flex items-center gap-1.5 truncate text-xs text-muted-foreground">
            ...
            {contactTags.length > 0 && (
              <ContactTags tags={contactTags} max={3} className="flex-none" />
            )}
          </p>
        )}
      </div>
